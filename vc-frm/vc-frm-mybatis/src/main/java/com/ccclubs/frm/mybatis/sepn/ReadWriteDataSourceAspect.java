...
@Aspect
public abstract class ReadWriteDataSourceAspect {
	...
	@Pointcut("@within(com.ccclubs.frm.mybatis.ReadWriteDataSource) && allMethodPoint()")
	...
	public void changeDataSourceBeforeMethodExecution(JoinPoint jp) {
		...
		// 没有配置实用默认数据源
		if (resultDS == null) {
			ReadWriteSqlSessionContextHolder.setSessionFactoryKey(null);
			return;
		}
		preDatasourceHolder.set(ReadWriteSqlSessionContextHolder.getDataSourceKey());
		// 将数据源设置到数据源持有者
		ReadWriteSqlSessionContextHolder.setSessionFactoryKey(resultDS);
	...
	public void restoreDataSourceAfterMethodExecution() {
		ReadWriteSqlSessionContextHolder.setSessionFactoryKey(preDatasourceHolder.get());
	...
	private String resolveDataSourceFromMethod(Class targetClass, String methodName) {
		Method[] mds = targetClass.getMethods();
		Method m = null;// ReflectUtil.findUniqueMethod(targetClass, methodName);
		for(Method mm : mds){
		...
		if (m != null) {
			ReadWriteDataSource choDs = m.getAnnotation(ReadWriteDataSource.class);
	...
	/**
	 * <li>方法描述 : 类级别的 @ChooseDataSource 的解析</li>
	 * 
	 * @param targetClass
	 * @return
	 */
	...
	private String resolveDataSourceFromClass(Class targetClass) {
		ReadWriteDataSource classAnnotation = (ReadWriteDataSource) targetClass.getAnnotation(ReadWriteDataSource.class);
		// 直接为整个类进行设置
	 ...
	 * @return
	 */
	private String resolveDataSourcename(ReadWriteDataSource ds) {
