...
import com.ccclubs.common.modify.UpdateRemoteService;
                ...
                case 0x10000000:
                    ...
                    switch (fcCodeClose) {
                        ...
                        case 0x10040000:
                            ...
                            csRemote = RemoteHelper.getRemote(commonWriter.mId, tm.getHexString(), resultJsonClose,
...
//          terminalPartStatus.setCssNumber(tm.getCarNumber());
                    terminalPartStatus
                            .setCssEvBattery(terminalInfo.getSoc() == null ? 0 : terminalInfo.getSoc());
                    terminalPartStatus
                            .setCssObdMile(terminalInfo.getObdMile() == null ? 0 : terminalInfo.getObdMile());
                    terminalPartStatus
                            .setCssLatitude(terminalInfo.getTriggerGpsLatitude());
                    terminalPartStatus
                            .setCssLongitude(terminalInfo.getTriggerGpsLongitude());
                    ...
                    terminalPartStatus.setCssCharging(terminalInfo.getTriggerChargeStatus() == null ? 0
                            : terminalInfo.getTriggerChargeStatus());
                    terminalPartStatus.setCssDoor(
                            terminalInfo.getTriggerMergeDoorStatusWithMask() == null ? 0
                                    : terminalInfo.getTriggerMergeDoorStatusWithMask());
                    terminalPartStatus.setCssEngine(terminalInfo.getTriggerEngineStatus() == null ? 0
                            : terminalInfo.getTriggerEngineStatus());
                    terminalPartStatus.setCssLock(terminalInfo.getTriggerDoorLockStatusWithMask() == null ? 0
                            : terminalInfo.getTriggerDoorLockStatusWithMask());
                    terminalPartStatus.setCssLight(terminalInfo.getTriggerLightStatusWithMask() == null ? 0
    ...
    private void modifyTerminalBle(MqMessage message) {
        ...
        if (bleModle != null) {
            ...
            csMachine.setCsmId(mapping.getMachine().intValue());
            // 更新终端mac地址
...
//      transferToMq(MqTagProperty.MQ_TERMINAL_ORDER, message);
            ...
            if (orderUpStream.isSuccess()) {
                jsonString = RemoteHelper.getRemoteJsonMessage(orderUpStream.mId);
            } else {
            ...
            CsRemote csRemote = RemoteHelper.getRemote(orderUpStream.mId, message.getHexString(), jsonString,
        ...
        try {
            ...
            if (topic.equals(srvHost.getShTopic())) {
                ...
                if (mqMessage != null) {
                    mqMessage
                            .setKey(
                                    message.getCarNumber() + StringUtils
                                            .date(new Date(), ConstantUtils.TIME_FORMAT_KEY));
