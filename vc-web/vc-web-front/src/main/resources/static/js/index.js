$(function(){
	$.getScript("/js/tpui-core.js", function(data, status, jqxhr) {
		...
		$.extend(vote , {
			...
			events : function(){
				$('#wrapper').on('click' , '.select-btn' , $.proxy( this.onemenulevel , this))
			...
			middleView: function(event){
				...
				var iframe = $('<iframe class="J_iframe" name="iframe'+index+'" width="100%" height="100%" src="'+url+'" frameborder="0" data-id="'+url+'" seamless></iframe>').appendTo($('#content-main'));
			},
			isShow:function(){
