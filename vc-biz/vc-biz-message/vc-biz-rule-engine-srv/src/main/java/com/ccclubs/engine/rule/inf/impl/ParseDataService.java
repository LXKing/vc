...
public class ParseDataService implements IParseDataService {
    ...
    public void processCarStatus(MqMessage message) {
        try {
            ...
            if (topic.equals(srvHost.getShTopic())) {
                transferToMq(MqTagProperty.MQ_TERMINAL_STATUS, message);
            } else {
                transferToMq(srvHost, transformUtils.transform2TerminalStatus(csMachine.getCsmTeNo(), mapping.getVin(), mqtt_66, message),
                        message);
            }
    ...
    public void processCanStatus(MqMessage message) {
        try {
            transferToMq(MqTagProperty.MQ_TERMINAL_CAN, message);
...
//            CsMachine csMachine = TerminalUtils.getCsMachineByNumber(message.getCarNumber());
//            // mqtt自有协议时，需要更新，排除808透传
//            if (csMachine != null && $.empty(message.getSimNo())) {
//              if (!$.empty(carStartAndStop.mCfxId.trim())) {
//                csMachine.setCsmTeNo(carStartAndStop.mCfxId.trim());
//              }
//              // @ 更新csMachine时，手机号必须为11位长度
//              if (!$.empty(carStartAndStop.mMobile.trim())
//                  && carStartAndStop.mMobile.trim().length() >= 11) {
//                csMachine.setCsmMobile(carStartAndStop.mMobile.trim().substring(0, 11));
//              }
//              csMachine.setCsmV1(Tools.ToHexString(carStartAndStop.mIapVersion) + Tools
//                  .ToHexString(carStartAndStop.mAppVersion));
//              csMachine.setCsmV2(Tools.ToHexString(carStartAndStop.mVersion));
...
//            }
                ...
                case 0x02:
                    ...
                    if (mapping == null || StringUtils.empty(mapping.getNumber()) || mapping.getMachine() == null
    ...
    public void processTakeCarStatus(MqMessage message) {
        try {
            transferToMq(MqTagProperty.MQ_TERMINAL_TAKECAR, message);
    ...
    public void processFurtherCarStatus(MqMessage message) {
        try {
            transferToMq(MqTagProperty.MQ_TERMINAL_FURTHERCAR, message);
    ...
    public void processRestart(MqMessage message) {
        try {
            ...
            if (!StringUtils.empty(mqtt_6B.getSuperSimNo())) {
                ...
                CsMachine csMachine = queryTerminalService.queryCsMachineById(mapping.getMachine().intValue());
    ...
    public void processTerminalInfo(MqMessage message) {
        try {
            ...
            // 转发车机属性信息
            transferToMq(MqTagProperty.MQ_TERMINAL_INFO, message);
            ...
            terminalUtils.processTerminalLog(
                    StringUtils.empty(terminalInfo.getSimNo()) ? message.getCarNumber() : terminalInfo.getSimNo(),
                    info, message.getHexString(),
                    message.getTransId());

            if (StringUtils.empty(message.getSimNo())) {
                // mqtt 终端，按照车机号唯一性来确认 --> 通过车机号来规范手机号，序列号 等
                CsMachine numberMachine = queryTerminalService.queryTerminalByCarNumber(message.getCarNumber());//.Get($.add(CSM.csmNumber, message.getCarNumber()));
                ...
                CsMachine mobileMachine = queryTerminalService.queryCsMachineBySimNo(terminalInfo.getSimNo());
     ...
     */
    private void transferToMq(String tag, MqMessage message) {
        try {
            Message mqMessage = messageFactory
                    .getMessage(topic, tag, message);

            if (mqMessage != null) {
                mqMessage.setKey(message.getCarNumber() + "_" + StringUtils.date(new Date(), ConstantUtils.TIME_FORMAT));
                client.sendOneway(mqMessage);
            } else {
                logger.error(message.getCarNumber() + " 未授权给应用");
            }
