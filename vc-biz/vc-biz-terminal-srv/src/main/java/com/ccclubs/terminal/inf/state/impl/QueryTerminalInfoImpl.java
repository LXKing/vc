...
public class QueryTerminalInfoImpl implements QueryTerminalInfoInf {
    ...
    public TerminalQryOutput getTerminalInfo(TerminalQryInput input) {
        ...
        switch (csMachine.getCsmTeType()) {
            case 0://富士康
                output.setSoftVersion(csMachine.getCsmV1() == null || "".equals(csMachine.getCsmV1()) ? "" : Integer.toString(Integer.parseInt(csMachine.getCsmV1().substring(4), 16)));
                output.setHardVersion(csMachine.getCsmV2());
                output.setPluginVersion(csMachine.getCsmV1() == null || "".equals(csMachine.getCsmV1()) ? "" : Integer.toString(Integer.parseInt(csMachine.getCsmV1().substring(4), 16)));
                break;
            case 1://中导
                output.setSoftVersion(csMachine.getCsmTlV1());
                output.setHardVersion(csMachine.getCsmV2());
                output.setPluginVersion(csMachine.getCsmTlV2() == null || "".equals(csMachine.getCsmTlV2()) ? "" : csMachine.getCsmTlV2().toString());
                break;
            case 2://慧瀚
                break;
            case 3://通领
    ...
    public VersionQryOutput isLatestVersion(VersionQryInput input) {
        ...
        // 未查询到车辆
        if (list.size() != 1) {
            throw new ApiException(ApiEnum.VEHICLE_NOT_FOUND, input.getVin());
        }
...
//        }
    ...
    private VersionQryOutput makeFSKOutput(String latest_version, String latest_filename, VersionQryOutput output, CsMachine csMachine) {
        if (null == csMachine.getCsmV1()) {
            ...
            if (latest_version.equals(Integer.toString(Integer.parseInt(csMachine.getCsmV1().substring(4), 16)))) {
                output.setLatest(true);
                output.setTerminalType(csMachine.getCsmTeType());
                output.setLatestV(latest_version);
                output.setFilename(latest_filename);
                output.setCurrentV(Integer.toString(Integer.parseInt(csMachine.getCsmV1().substring(4), 16)));
            } else {
                output.setLatest(false);
                output.setTerminalType(csMachine.getCsmTeType());
                output.setLatestV(latest_version);
    ...
    private VersionQryOutput makeOutput(String latest_version, String latest_filename, VersionQryOutput output, CsMachine csMachine) {
