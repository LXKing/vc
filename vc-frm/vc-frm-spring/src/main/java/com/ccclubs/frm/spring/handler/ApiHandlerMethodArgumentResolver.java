...
public class ApiHandlerMethodArgumentResolver implements HandlerMethodArgumentResolver {
	...
	public Object resolveArgument(MethodParameter parameter, ModelAndViewContainer mavContainer,
		...
		log.debug("sign:{}, v:{}, apiId:{}, apiName:{}", sign, v, appId, apiName);
		InputStream is = request.getInputStream();
		String jsonContents = IOUtils.toString(is, "UTF-8");
		log.debug("s:{}", jsonContents);
		...
		if (StringUtils.isEmpty(sign) || StringUtils.isEmpty(calc_sign) || !calc_sign.equals(sign)) {
			throw new ApiException(ApiEnum.SIGN_CHECK_FAILED.code(), ApiEnum.SIGN_CHECK_FAILED.msg());
		}
		...
		//JavaType javaType = objectReader.getTypeFactory().constructSimpleType(cls, new JavaType[] { secondJavaType });
