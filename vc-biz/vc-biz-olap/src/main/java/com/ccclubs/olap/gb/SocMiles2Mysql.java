...
public class SocMiles2Mysql {
    private static Logger logger = Logger.getLogger(SocMiles2Mysql.class);
        ...
        for(int i=0;i<waitList.size();i++) {
            ...
            if (i >= batchStartNo && i < batchEndNo) {
                ...
                String sql_str = " select " +
                        ...
                        "a.end_time, " +
                        "a.pace_timemills " +
                        "from  dw_soc_miles_basic a  where " + partition_sql+" "+
                        "distribute by a.cs_vin sort by a.start_time asc ";

                Dataset<Row> result_df = sparkSession.sql(sql_str);

                JavaPairRDD<String,Row> pairRDD = result_df.toJavaRDD().mapToPair(new PairFunction<Row, String, Row>() {
                    @Override
                ...
                pairRDD.partitionBy(new CsVinPartitioner(chsMap))
                    ...
                    public Iterator<Row> call(Iterator<Row> rowIterator) throws Exception {
                        ...
                        while (rowIterator.hasNext()) {
                            ...
                            int changed_soc = row.getAs("changed_soc");
                            ...
                            socMiles.setChanged_soc(changed_soc);
                        ...
                        BIReport.generateSocReportToMysql(socMilesList);
                        return rowIterator;
                    }
                }).take(1);
            }
